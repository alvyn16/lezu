...
LEZU is local-first. It has no analytics, advertising, telemetry, account
system, or required online service.

## Local data

LEZU reads Steam library manifests, artwork caches, playtime, recent-play
...
within the save locations shown by LEZU.

LEZU retains:

- Library, source records, favorites, hidden state, and achievements in
  `$XDG_DATA_HOME/LEZU/library.sqlite3`
...
- Artwork retained for repair undo under `$XDG_DATA_HOME/LEZU/artwork/review-undo/`
- Optional RomM server URL and local mount path in settings, and its cached catalog
  in `$XDG_DATA_HOME/LEZU/romm-catalog.sqlite3`
...
- Play sessions recorded by `LEZU-sessiond` in the same database: game
...
- Steam ID, RetroAchievements username, public IGDB client ID, cache limit, and
  ...
  `$XDG_CONFIG_HOME/LEZU/config.toml`
- Downloaded covers and achievement icons in `$XDG_CACHE_HOME/LEZU/`
- SteamGridDB portraits in `$XDG_DATA_HOME/LEZU/portrait-covers/`
- Copies of covers, heroes, and logos selected by the user in
  `$XDG_DATA_HOME/LEZU/artwork/`
...
`io.github.tsouth89.LEZU.Steam`. Older preview keys stored as
`io.github.LEZU.Steam` remain readable. The key is never written to LEZU's config, database,
logs, or process arguments.

A RetroAchievements Web API key is stored only through the desktop Secret
Service under `io.github.tsouth89.LEZU.RetroAchievements`. It is never
written to LEZU's config, database, logs, or process arguments.

Optional IGDB game insights require a Twitch developer client ID and client
secret supplied by the user. The public client ID is stored in LEZU's config.
The client secret is stored through Secret Service as
`io.github.tsouth89.LEZU.IGDB` and is never written to config, the database,
logs, or process arguments. LEZU sends these credentials to Twitch only to
...
`io.github.tsouth89.LEZU.SteamGridDB`. It is never written to config,
...
under `io.github.tsouth89.LEZU.RomM`, scoped by server. The token is not written
...
read from the configured local mount; LEZU does not download games or upload saves.
...
When save protection is enabled, LEZU reads mapped emulator save files immediately
before launch and stores local versions under
`$XDG_DATA_HOME/LEZU/save-backups/`. These copies are not uploaded or included in
...
supported preferences. It excludes API credentials, LEZU account-service
...
LEZU may request missing covers and achievement icons from Steam's public
...
Steam Web API requests occur only after the user stores a key. LEZU refreshes
...
Library. LEZU requests the public game list and playtime for the configured
...
credentials. LEZU maps a Steam App ID to an IGDB game, then requests IGDB's
...
Web API key. LEZU downloads supported game hashes and matches ROM hashes
...
RetroAchievements, IGDB, SteamGridDB, or RomM credentials from Secret Service. Removing LEZU does not remove its XDG
data by default, so users can preserve settings across reinstallations.
Resetting a custom artwork slot removes its unused LEZU copy and restores
the source-provided artwork. It does not change the original selected image.

## Console keys

To show the icon and name stored inside a Switch dump, LEZU reads the
...
LEZU's config or database, logged, or sent anywhere. Without those files
Switch artwork falls back to files next to the dump and to Ryujinx's own
covers folder.

## Cover downloads

When a card becomes visible and no local artwork exists, LEZU may fetch a
...
id is sent. Downloads are cached under `~/.cache/LEZU/covers` and a system
...
Responses are size-limited and cached under LEZU's cache directory for seven
