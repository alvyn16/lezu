...
QStringList DolphinScanner::discoverRoots() {
  ...
  if (!appData.isEmpty()) {
    candidates.append(QDir::fromNativeSeparators(appData) + QStringLiteral("/Dolphin Emulator"));
  }
  candidates.append(QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + QStringLiteral("/Dolphin Emulator"));
#else
  const QString home = QDir::homePath();
  candidates = {
...
#ifdef Q_OS_WIN
  return !QStandardPaths::findExecutable(QStringLiteral("Dolphin")).isEmpty() ||
         !QStandardPaths::findExecutable(QStringLiteral("dolphin")).isEmpty() ||
