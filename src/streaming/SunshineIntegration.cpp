...
#if defined(Q_OS_WIN)
    m_imageRoot = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) +
                  QStringLiteral("/sunshine");
#else
    m_imageRoot = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) +
                  QStringLiteral("/lezu/sunshine");
#endif
  }
...
QString SunshineIntegration::commandPrefix(bool flatpakSunshine) {
  ...
  if (flatpakSunshine) {
    return QStringLiteral("flatpak-spawn --host lezu");
  }
  const QString executable = QStandardPaths::findExecutable(QStringLiteral("lezu"));
...
QJsonObject SunshineIntegration::LEZUEntry(const QString& prefix, const QString& imagePath) {
  ...
  QJsonObject entry;
  entry.insert(QStringLiteral("name"), QStringLiteral("Lezu"));
...
SunshineIntegration::SyncResult SunshineIntegration::runSync(
  ...
  if (!unchanged) {
    const QString backup = appsPath + QStringLiteral(".lezu-backup");
  ...
  if (!result.wrote) {
    ...
    result.status = QStringLiteral("Removed Lezu from Sunshine. Restart Sunshine to apply.");
  } else {
    result.status =
