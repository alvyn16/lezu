...
int main(int argc, char* argv[]) {
  ...
  QIcon applicationIcon(QStringLiteral(":/icons/resources/icons/lezu.ico"));
  if (applicationIcon.isNull()) {
    applicationIcon = QIcon::fromTheme(QStringLiteral("io.github.lezu.Lezu"));
  }
  if (applicationIcon.isNull()) {
    applicationIcon =
  ...
  if (!isolatedTest && !singleInstance.claimOrNotify(instanceCommand)) {
   ...
   const QString settingsPath =
       isolatedTest
           ? QDir::tempPath() +
                 QStringLiteral("/lezu-test-%1.toml").arg(QCoreApplication::applicationPid())
           : QString{};
   ...
   const BackupRecovery::Paths backupPaths{dataRoot + "/library.sqlite3",
                                           configRoot + "/config.toml",
                                           dataRoot + "/restore-recovery"};
#else
   const QString dataRoot =
   ...
   const BackupRecovery::Paths backupPaths{dataRoot + "/lezu/library.sqlite3",
                                           configRoot + "/lezu/config.toml",
                                           dataRoot + "/lezu/restore-recovery"};
#endif
...
#ifdef Q_OS_WIN
  std::unique_ptr<EpicGameModel> epicGames;
  std::unique_ptr<GogGameModel> gogGames;
#endif
...
#ifdef Q_OS_WIN
  std::unique_ptr<WinSessionWorker> winSessionWorker;
#endif
  SteamGameModel* steamLibrary = nullptr;
  LutrisGameModel* lutrisLibrary = nullptr;
  HeroicGameModel* heroicLibrary = nullptr;
#ifdef Q_OS_WIN
  EpicGameModel* epicLibrary = nullptr;
  GogGameModel* gogLibrary = nullptr;
#endif
...
#ifdef Q_OS_WIN
    epicGames = std::make_unique<EpicGameModel>(steamLibrary->databasePath());
    epicLibrary = epicGames.get();
    gogGames = std::make_unique<GogGameModel>(steamLibrary->databasePath());
    gogLibrary = gogGames.get();
    gogLibrary->setLibraryPaths(preferences.gogLibraryPaths());
    QObject::connect(&preferences, &AppSettings::gogLibraryPathsChanged, gogLibrary,
                     [&preferences, gogLibrary] {
                       gogLibrary->setLibraryPaths(preferences.gogLibraryPaths());
                       if (preferences.gogEnabled()) gogLibrary->refresh();
                     });
#else
...
#ifdef Q_OS_WIN
  if (epicGames != nullptr) {
    unifiedGames.addSourceModel(epicGames.get());
  }
  if (gogGames != nullptr) {
    unifiedGames.addSourceModel(gogGames.get());
  }
#else
...
#ifdef Q_OS_WIN
    unifiedGames.setSourceEnabled(QStringLiteral("Epic"), preferences.epicEnabled());
  ...
  if (!playKey.isEmpty()) {
    ...
    if (PlayRequest::findInstallation(unifiedGames, key, nullptr).isEmpty() && key.isValid()) {
      ...
      if (key.source.compare(QStringLiteral("Steam"), Qt::CaseInsensitive) == 0 &&
        ...
        refreshStarted = true;
#ifdef Q_OS_WIN
      } else if (key.source.compare(QStringLiteral("Epic"), Qt::CaseInsensitive) == 0 &&
                 epicLibrary != nullptr && preferences.epicEnabled()) {
        epicLibrary->refresh();
        refreshStarted = true;
      } else if (key.source.compare(QStringLiteral("GOG"), Qt::CaseInsensitive) == 0 &&
                 gogLibrary != nullptr && preferences.gogEnabled()) {
        gogLibrary->refresh();
        refreshStarted = true;
#else
...
#ifdef Q_OS_WIN
  engine.rootContext()->setContextProperty(QStringLiteral("EpicLibrary"), epicLibrary);
  engine.rootContext()->setContextProperty(QStringLiteral("GogLibrary"), gogLibrary);
#endif
...
#ifdef Q_OS_WIN
  if (!isolatedTest && preferences.trackPlaySessions()) {
    winSessionWorker = std::make_unique<WinSessionWorker>(libraryDatabasePath, QStringLiteral(":/resources/sessiond-profiles.json"));
    winSessionWorker->start();
  }
  QObject::connect(&preferences, &AppSettings::trackPlaySessionsChanged, [&preferences, &winSessionWorker, libraryDatabasePath] {
    if (preferences.trackPlaySessions()) {
      if (!winSessionWorker) {
        winSessionWorker = std::make_unique<WinSessionWorker>(libraryDatabasePath, QStringLiteral(":/resources/sessiond-profiles.json"));
      }
      winSessionWorker->start();
    } else if (winSessionWorker) {
      winSessionWorker->stop();
    }
  });
#endif
                   ...
                   [&retroArchLibrary, &pcsx2Library, &ryujinxLibrary, &dolphinLibrary,
                    &preferences](const QString& source) {
                     ...
                     if (source == QStringLiteral("Ryujinx") && ryujinxLibrary != nullptr &&
                       ...
                       dolphinLibrary->refresh();
#ifdef Q_OS_WIN
                     } else if (source == QStringLiteral("Epic") && epicLibrary != nullptr &&
                                preferences.epicEnabled()) {
                       epicLibrary->refresh();
                     } else if (source == QStringLiteral("GOG") && gogLibrary != nullptr &&
                                preferences.gogEnabled()) {
                       gogLibrary->refresh();
#endif
                     }
                   });
...
#ifdef Q_OS_WIN
  if (epicLibrary != nullptr && preferences.epicEnabled()) {
    QTimer::singleShot(250, epicLibrary, &EpicGameModel::refresh);
  }
  if (gogLibrary != nullptr && preferences.gogEnabled()) {
    QTimer::singleShot(350, gogLibrary, &GogGameModel::refresh);
  }
#else
