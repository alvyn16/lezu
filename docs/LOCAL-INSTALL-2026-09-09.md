...
The exact source commit, executable paths, SHA-256 checksums, previous executable, and installation time are recorded in `/home/bts/.local/state/LEZU/local-install-20260909/installed-candidate.json` and the installed build's `candidate.json`.
...
The original checkout's earlier patches and quality-plan draft are preserved in stash `9c457db7e78cae47eb650d29b716974f0543ca7d`. A separate patch copy, the old 1.7.1 package archive, settings/desktop backups, and a consistent pre-install library backup are in `/home/bts/.local/state/LEZU/local-install-20260909/` (owner-only directory).

The previous application remains at `/home/bts/.local/lib/LEZU/5ba99e6/LEZU`. To roll back, close LEZU, restore the application command symlink and saved desktop entry, remove the newly added `~/.local/bin/LEZU-sessiond` symlink, remove only `~/.config/systemd/user/LEZU-sessiond.service.d/local-build.conf`, reload user systemd, and restart the recorder. This returns the recorder to the packaged executable. Do not restore the database backup over newer play history merely to change application binaries.
