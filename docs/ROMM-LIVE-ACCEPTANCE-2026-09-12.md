...
September 12, 2026. Tested LEZU's installed `3afe8190c90f` candidate against an
actual local RomM 5.2.0 server. No LEZU runtime fix was required.
...
- Disposable local library containing `roms/snes/LEZU QA.sfc`.
- Dedicated test administrator and Client API Tokens with only `roms.read`.
- Isolated LEZU settings, cache, database, runtime directory, and temporary directory.
...
| Check | Result |
| --- | --- |
| Connect through LEZU's actual RommGameModel and desktop keyring | Pass |
...
| Stop server, recreate LEZU model, retain cached identity and favorite | Pass |
...
| Installed LEZU `--play` with empty local cache fetches real catalog and hands correct file to controlled process | Pass |
...
containers, personal library settings, saves, and installed LEZU binaries were preserved.

The opt-in test skips during ordinary CTest runs. To repeat it, provision the dedicated
local server described above, name its app container `LEZU-romm-qa-romm-1`, mount the
one-entry library at `/tmp/LEZU-romm-live/library`, and create a `roms.read` token.
Save its creation response privately as JSON containing `raw_token`, then run:

```sh
LEZU_LIVE_ROMM_TOKEN_FILE=/path/to/private-token.json \
  QT_QPA_PLATFORM=offscreen \
  build/release/tests/LEZU_romm_catalog_tests liveServerAcceptance
```
