...
Profiles recognize emulator processes and game paths in command-line arguments.
A listed profile is not evidence that every launcher, wrapper, or internal game
change works. Recording does not inject code, control emulators, or detect pauses.
...
recognizable command-line path is not covered. ARM64 hardware acceptance remains
separate from cross-architecture build and package checks.
