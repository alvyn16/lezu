...
ColumnLayout {
    ...
    FileDialog {
        id: filesDialog
        title: "Choose explicit save files"
        fileMode: FileDialog.OpenFiles
        onAccepted: {
            customFiles.text = selectedFiles.map(function(url) {
                const raw = decodeURIComponent(url.toString().replace(/^file:\/\//, ""));
                return (Qt.platform.os === "windows" && raw.match(/^\/[a-zA-Z]:/)) ? raw.substring(1) : raw;
            }).join("\n");
        }
    }
}
