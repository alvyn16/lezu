...
ColumnLayout {
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            CheckBox { id: flatpak; palette.windowText: Theme.foreground; text: "Use Flatpak"; Accessible.name: text; visible: Qt.platform.os !== "windows" }
            RowLayout {
    ...
    FileDialog {
        id: filePicker
        property bool forCore: false
        title: forCore ? "Choose a libretro core" : "Locate the game"
        nameFilters: forCore ? ["Libretro cores (*_libretro.dll *_libretro.so *.dll *.so)", "All files (*)"] : ["Game files (*)"]
        onAccepted: {
            const raw = decodeURIComponent(selectedFile.toString().replace(/^file:\/\//, ""));
            const path = (Qt.platform.os === "windows" && raw.match(/^\/[a-zA-Z]:/)) ? raw.substring(1) : raw;
            if (forCore) core.text = path; else location.text = path;
        }
    }
}
