...
Rectangle {
    ...
    function summary() {
        ...
        const labels = {reduced_motion:"Reduced motion", artwork_cache_limit_mb:"Artwork cache limit (MB)", steam_enabled:"Steam", epic_enabled:"Epic", gog_enabled:"GOG", retroarch_enabled:"RetroArch", pcsx2_enabled:"PCSX2", ryujinx_enabled:"Ryujinx", pcsx2_auto:"Detect PCSX2 automatically", ryujinx_auto:"Detect Ryujinx automatically", battlenet_enabled:"Battle.net", close_after_launch:"Close after launching", couch_mode:"Couch Mode", couch_library_view:"Couch library view", gog_library_paths:"GOG folders",
    ...
    FileDialog {
        id: saveDialog
        title: "Save a Lezu backup"
        fileMode: FileDialog.SaveFile
        defaultSuffix: "lezu-backup"
        nameFilters: ["Lezu backups (*.lezu-backup)"]
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            TextField {
                ...
                Layout.fillWidth: true
                placeholderText: "C:/path/to/library.lezu-backup"
        ...
        Text {
            ...
            text: editor.pendingMode === "export" ? "Save a backup to " + editor.pendingPath + "? An existing file at this path will be replaced."
                  : "Restore " + Backups.preview.path + "?\n\n" + (editor.pendingMode === "replace" ? Backups.preview.replaceExplanation + " Core preferences absent from the backup return to defaults." : Backups.preview.mergeExplanation)
                    + "\n\nLezu will keep a recovery copy and apply this restore after you close and reopen the app."
        ...
        GlassButton { id: closeAppButton; objectName: "backupCloseAppButton"; visible: editor.queued; text: "CLOSE LEZU"; displayScale: editor.uiScale; onClicked: Qt.quit() }
